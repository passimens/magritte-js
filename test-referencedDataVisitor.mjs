...
import { MAReferencedDataHumanReadableSerializer } from './Magritte/visitors/MAReaderWriterWalkerVisitors.js';
import { MAReferencedDataHumanReadableDeserializer } from './Magritte/visitors/MAReaderWriterWalkerVisitors.js';
...
import { PrintVisitor, printModel } from './test-printVisitor.mjs';
  ...
  const ipDescriptor_orig = hostDescriptor.children.find((element) => element.name == 'address');
  const ipDescriptor = Object.assign(Object.create(Object.getPrototypeOf(ipDescriptor_orig)), ipDescriptor_orig);
  ipDescriptor._accessor = 'MAIdentityAccessor';
  const portsDescriptor_orig = hostDescriptor.children.find((element) => element.name == 'ports');
  const portsDescriptor = Object.assign(Object.create(Object.getPrototypeOf(portsDescriptor_orig)), portsDescriptor_orig);
  portsDescriptor._accessor = 'MAIdentityAccessor';
  ...
  console.log(" === test-print models === ")
  printModel(hostDescriptor, host);
  printModel(portDescriptor, port);
  console.log(" === test-referencedDataVisitor === ")
  ...
  console.log("dto_h: ");
  console.log(dto_h);
  console.log(" --- printing dto_h model ---")
  printModel(hostDescriptor, dto_h);
  console.log(" --- printing dto_h.ports model ---")
  ...
  console.log("dto_p: ");
  console.log(dto_p);
  console.log(" --- printing model dto_p ---");
  ...
  console.log(dto_ports);
  console.log("host.ports.length: ");
  console.log(host.ports.length);
  console.log("dto_ports.length: ");
  console.log(dto_ports.length);
  ...
  const dto_test_datetime = deserializer.deserializeHumanReadable(JSON.stringify(test_datetime_dump), new MADateAndTimeDescription({"accessor":"MAIdentityAccessor"}));
  const serialized_str_test_datetime = serializer.serializeHumanReadable(dto_test_datetime, new MADateAndTimeDescription({"accessor":"MAIdentityAccessor"}));
  console.log(test_datetime_dump, dto_test_datetime, serialized_str_test_datetime);

  const dto_test_date = deserializer.deserializeHumanReadable(JSON.stringify(test_date_dump), new MADateDescription({"accessor":"MAIdentityAccessor"}));
  const serialized_str_test_date = serializer.serializeHumanReadable(dto_test_date, new MADateDescription({"accessor":"MAIdentityAccessor"}));
